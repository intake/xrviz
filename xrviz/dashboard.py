...
class Dashboard(SigSlot):
    ...
    data: xarray.DataSet or xarray.DataArray
        Is passed to the user input panes for initialization.
    ...
    def create_indexed_graph(self, *args):
        """
        Creates a graph for the dimensions selected in `x` and `y`.

        This is used when values selected in `x` and `y` are not data coordinates (i.e. 
        one or both values are data dimensions).
        """
